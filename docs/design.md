...
基底采用 `radial_decay = (1 - r_norm)^1.3` 的单调递减剖面，再叠加少量 FBM 噪声，钳制在 0~0.25，确保“内热外冷”。
各结构在生成密度的同时贡献独立的温度偏移，每个实例有随机 ΔT：

| 结构 | ΔT 分布 | 物理依据 |
|------|---------|----------|
| 螺旋臂 | uniform(0.1, 0.3) | 密度波压缩加热 |
| Hotspot | 0.5 + 2.5 × power(0.4)，幂律分布 | 磁重联（小耀斑多、大爆发少） |
| Filament | uniform(0.05, 0.15) | 剪切摩擦加热 |
| RT spike | uniform(0.3, 1.0) | ISCO 附近高温物质 |
| 云雾 | 0.05 × turbulence | 湍流剪切带来的粘滞加热 |

合成流程：
1. 将 `temp_struct` 按 95% 分位归一，再乘 0.8，限制在 0~1.2；
2. 逐半径计算 `struct_p70(r)`（70% 分位）与 `struct_max(r)`，基底上限先取 `max(struct_p70, 0.05)`，再确保 `≤ struct_max(r)`；
3. `temperature_field = clip(max(T_base_clamped, temp_struct_scaled), 0, 1)`，满足“颜色由最高温度决定”。

**密度场**：

1. **螺旋臂**（2-4 条）：
   - 2-3 条从圆心开始，其余从随机半径开始
   - 每条旋转 2.5-5 圈，宽 0.2-0.4（von Mises 分布，kappa≈1.5/width²）
   - 噪声调制宽度和强度，产生断断续续、时宽时窄的效果

2. **云雾**（多频弧线叠加 + 开普勒剪切扭曲）：
   - 5 层频率叠加（8/24/80/200/400）+ 像素级噪声
   - 中低频层应用开普勒剪切扭曲（φ 偏移 ∝ r^(-1.5)），拉出方向性丝状结构
   - 模拟差分旋转对云雾的拉伸效果

3. **Filaments**（80-150 × disk_area 条细丝）：
   - 模拟 Kelvin-Helmholtz 不稳定性
   - 内圈更密集更短，外圈更长更稀疏

4. **热点**（25-45 × disk_area 个弧形亮斑）：
   - von Mises 角度分布 + 高斯径向分布
   - 角度宽度 0.08-0.2，径向宽度 0.005-0.02
   - 内圈更密集更亮（磁重联活动更频繁）

5. **RT 不稳定性**（ISCO 附近尖峰和团块）

密度组合：`0.08*底色 + 0.18*螺旋臂 + 0.42*云雾 + 0.14*热点 + 0.08*细丝 + 0.15*RT`
